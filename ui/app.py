...
# ─────────────────────────────────────────────────────────────────────────────
# Core Logic & UI Renderers
# ─────────────────────────────────────────────────────────────────────────────

def get_new_env():
    """Initializes a fresh environment state for the session."""
    return ParkingEnv()

def render_grid(obs):
    """Generates the visual HTML for the parking lot."""
    if not obs:
...
def render_queue(obs):
    """Generates the visual HTML for the incoming car queue."""
    if not obs:
...
def update_dropdowns(obs):
    """Syncs dropdown options with the current available queue and slots."""
    if not obs:
...
def toggle_inputs(action_type):
    """Enables/disables UI inputs depending on the selected action constraint."""
...
def execute_action(env, log_history, action_type, car_id, slot_id):
    ...
    try:
        ...
        # Construct strictly valid actions per Pydantic validators
        if act_enum == ActionType.WAIT:
            act = Action(action_type=act_enum, car_id=None, slot_id=None)
        elif act_enum == ActionType.REJECT:
            if car_id is None:
                raise ValueError("Must select a car to REJECT.")
            act = Action(action_type=act_enum, car_id=car_id, slot_id=None)
        else:
            if car_id is None or slot_id is None:
                raise ValueError("Must select both a car and a slot to ASSIGN.")
        ...
        entry += f"<br><span style='font-size: 0.9em; color: #cbd5e0;'>{info_msg}</span>"
        entry += "</div>"
        
        # Append final grade if task completes
        if res.done:
            summary = env.summary()
            entry = f"<div style='background-color: #276749; padding: 10px; border-radius: 5px; margin: 10px 0; color: white;'>" \
                    f"<b>🎉 Episode Finished!</b><br>" \
                    f"Final Score: <b>{summary.final_score:.2f} / 1.0</b><br>" \
                    f"Total Revenue: ${summary.total_revenue:.2f}<br>" \
                    f"Cars Parked: {summary.cars_parked} | Rejected: {summary.cars_rejected} | Invalid Actions: {summary.invalid_actions}" \
                    f"</div>" + entry
            
        new_log = entry + log_history
        return env, grid, queue, car_u, slot_u, new_log
...
def execute_wait(env, log_history):
    """Convenience wrapper for the dedicated WAIT button."""
...
def build_app():
    ...
    with gr.Blocks(title="Smart Parking Lot", theme=gr.themes.Soft(primary_hue="blue", neutral_hue="slate")) as app:
        gr.Markdown(
            """
            <div style="text-align: center; padding: 20px;">
                <h1>🅿️ Smart Parking Lot</h1>
                <p style="color: gray; font-size: 1.1em;">OpenEnv-compatible simulation · Agentic AI Hackathon Demo</p>
            </div>
            """
        )
        ...
        reset_btn.click(
            ...
            outputs=[env_state, grid_display, queue_display, car_dropdown, slot_dropdown, log_display]
        )
        
        action_type_radio.change(
            fn=toggle_inputs,
            inputs=action_type_radio,
            outputs=[car_dropdown, slot_dropdown]
        )
        
        exec_btn.click(
            fn=execute_action,
            inputs=[env_state, log_display, action_type_radio, car_dropdown, slot_dropdown],
            outputs=[env_state, grid_display, queue_display, car_dropdown, slot_dropdown, log_display]
        )
        
        wait_btn.click(
            fn=execute_wait,
            inputs=[env_state, log_display],
            outputs=[env_state, grid_display, queue_display, car_dropdown, slot_dropdown, log_display]
        )
